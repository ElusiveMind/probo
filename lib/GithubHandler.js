...
GithubHandler.prototype.pullRequestHandler = function(event, done) {
  ...
  myRequest({
    url: "https://hooks.slack.com/services/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",
    method: "POST",
    json: true,
    body: {
      text: `Starting Build: ${event.payload.repository.name}: Pull Request: ${event.payload.pull_request.title}`
    }
  }, function (error, response, body){
    this.log.error({err: error}, 'Could not send message to Slack.');
  });
